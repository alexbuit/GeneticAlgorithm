...
ga.set_select(dfmc.selection_funcs.rank_space_selection) # Rank selection
...
runcond = r"np.min(np.abs(self.log.ranking.distancefx[-1])) > 1"  # Stop when the minimum distance from the best solution to the mathematical is less than 0.1
...
selfuncs = [dfmc.selection_funcs.rank_tournament_selection, dfmc.selection_funcs.roulette_selection]
...
for j in range(5):
    ...
    for i in range(100):
        try:
            ...
            epochs[i, 3] = ga.tfunc.calculations
            ga.reset(reset_pop=False)
            tstart = time.time()
        except Exception as e:
            pass

    np.savetxt("test%s.csv" % selfuncs[j].__name__, epochs, delimiter=";")
